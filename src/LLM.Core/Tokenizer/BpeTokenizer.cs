...
public sealed class BpeTokenizer
{
    ...
    /// <summary>Total number of byte, merge, and special tokens.</summary>
    ...
    /// <summary>Appends this tokenizer's EOS token to an already encoded document.</summary>
    public int[] EncodeDocument(ReadOnlySpan<byte> data)
    {
        int eos = _eosTokenId ?? throw new InvalidOperationException("Tokenizer has no EOS token.");
        int[] text = Encode(data);
    ...
    /// <summary>Serializes the tokenizer; byte and merge vocabulary entries are derived.</summary>
    ...
    {
        if (_eosTokenId.HasValue) throw new InvalidOperationException("Tokenizer already has an EOS token.");
        _eosTokenId = _vocab.Count;
        _vocab.Add(Array.Empty<byte>());
    }
